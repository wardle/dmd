(ns com.eldrix.dmd.graph
  "Provides a graph API around UK dm+d structures, built on pathom3.

  pathom3 is an optional dependency: it is deliberately not declared in the
  library's `:deps` so that consumers who do not use this namespace pay no
  cost. To use this namespace, add `com.wsscode/pathom3` to your own
  dependencies; loading this namespace without it on the classpath will fail."
  (:require
